...
function DailySpendingPage({ transactions = [], selectedDate, onBack, currentDay }) {
  ...
  const categoryColors = {
    shopping: { bg: '#FFB5E8', name: 'Shopping' },
    food: { bg: '#B5E8B5', name: 'Food' },
    transport: { bg: '#B5D4FF', name: 'Transport' },
    entertainment: { bg: '#FFE8B5', name: 'Entertainment' }
  };
          ...
          {weekDays.map((day, index) => {
            ...
            return (
              <div key={day} className="flex flex-col items-center">
                ...
                <button
                  onClick={() => {
                    if (date <= actualCurrentDay && onBack) {
                  ...
                  disabled={date > actualCurrentDay}
                  className={`w-10 h-10 rounded-full flex items-center justify-center text-sm font-medium transition-all ${
                    isSelected 
                      ? 'bg-black text-white' 
                      : date > actualCurrentDay 
                      ? 'bg-gray-100 text-gray-400 cursor-not-allowed'
                      : 'bg-[#FFF6F7] text-black hover:bg-gray-200'
                  }`}
                >
                  {date}
                </button>
